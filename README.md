...
The documentation workflow publishes versioned MkDocs output to the `gh-pages` branch with `mike`; GitHub Pages should be configured to serve the `gh-pages` branch root. If `nightly` is deployed before `stable`, it temporarily becomes the default channel.
